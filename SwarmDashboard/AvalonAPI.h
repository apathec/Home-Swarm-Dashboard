...
public:
  static bool fetch(const char* host, AvalonReading& out, uint16_t timeoutMs = 2500) {
    ...
    // ---- Hashrate: read the raw "MHS 5s" instant reading.
    // The dashboard side averages multiple readings across cycles
    // for a smooth display.
    // MHS values are in MH/s. Convert MH/s -> GH/s by dividing 1000.
    float mhs = parseEq(s, "MHS 5s");
    if (mhs == 0) mhs = parseEq(s, "MHS 1m");
